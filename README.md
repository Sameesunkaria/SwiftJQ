...
`SwiftJQ` is currently only supported on macOS 10.12+, iOS 10.0+, tvOS 10.0+ and watchOS 3.0+. Additionally, `SwiftJQ` imports binary dependencies in the form of XCFrameworks, and hence require Xcode 12 or higher. Due to some bugs related to code signing, currently this package is only supported on Xcode 12.5 beta 3 or higher. You may use this package on Xcode 12.4, but it may require you to enable to the `--deep` code signing flag.

## Usage

The `SwiftJQ` package exposes a `JQ` class, which is used for evaluating any jq program. Running a program is done in two steps:
...
Initialize a `JQ` object with the jq program. During this stage the program is compiled. Any compilation errors that may occur are thrown here (see `JQ.InitializationError`).
...
`SwiftJQ` lets you apply addition output formatting options to the JSON resulting from processing an input. These options are available on the `OutputConfiguration` struct and correspond to some the jq command line flags. The available output configurations are listed below:
...
However, generally the initialization/compilation step is considered to be a slow step while, processing an input is relatively fast. It is not recommended to perform the initialization on the main thread, as it may lead to some frames being dropped, if the application is presenting a UI.

Once a `JQ` object is initialized, it may be used to process an input from any thread/queue. However, currently (since a `JQ` object is backed by a single `jq-state` from the jq c library) only one input can be processed at a time. If you want to process multiple inputs in parallel, please initialize a new instance of the `JQ` object for each thread. Attempting to processing multiple inputs in parallel may lead to the caller having to wait while another input finishes processing.
...
- **`halt_error`**: Fails processing the input and throws a `JQ.ProcessingError.halt` error.
- **`halt_error/1`**: Fails processing the input and throws a `JQ.ProcessingError.halt` error, except if the message (input) is `null` and the exit code (first argument) is `0`, where it is treated as a `halt` instead.
